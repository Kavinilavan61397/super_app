...
function RestaurentPageCategory() {
    ...
    const { restaurentCategoryName } = useParams();
    ...
    useEffect(() => {
        const getRestaurents = async () => {
            try {
                const response = await axios.get(`https://yrpitsolutions.com/ecom_backend/api/get_restaurant_categories_by_name/${restaurentCategoryName}`);
        ...
        if (restaurentCategoryName) {
            getRestaurents();
        }
    }, [restaurentCategoryName]);
        ...
        <div className='min-h-screen'>
            ...
            <div className='pt-24 pb-28 px-4 bg-white'>
                ...
                <div className="grid grid-cols-2 gap-4 w-full mt-2">
                    {restaurent.map((product) => (
                        <FoodProducts key={product.id} product={product} />
                    ))}
                </div>
            </div>
            <FooterFood />
