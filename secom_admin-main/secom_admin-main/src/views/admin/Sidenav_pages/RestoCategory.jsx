...
function RestoCategory() {
    ...
    useEffect(() => {
        if (selectedCategory) {
            ...
            setImagePreview(selectedCategory.category_image);
            setValue('vendor_id', selectedCategory.vendor_id);
        }
    ...
    const handleFormSubmit = async (data) => {
        ...
        const existingCategory = getPaginatedData().find(category => category.category_name.toLowerCase() === category_name.toLowerCase());
        ...
        if (data.category_image instanceof File) formData.append('category_image', data.category_image);
        
        try {
            const accessToken = localStorage.getItem('OnlineShop-accessToken');
            const url = 'https://yrpitsolutions.com/ecom_backend/api/admin/save_restaurant_categories'; // For new category
    ...
    const handleFormUpdate = async (data) => {
        ...
        try {
            const accessToken = localStorage.getItem('OnlineShop-accessToken');
            const url = `https://yrpitsolutions.com/ecom_backend/api/admin/update_restaurant_categories_by_id/${selectedCategory.id}`;
    ...
    const handleDeleteConfirmation = async () => {
        ...
        try {
            const accessToken = localStorage.getItem('OnlineShop-accessToken');
            const response = await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_categories_by_id/${rowIdToDelete}`, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            for (let id of selectedRows) {
                await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_categories_by_id/${id}`, {
                    headers: { Authorization: `Bearer ${accessToken}` },
                });
            }
                ...
                {openAddModal && (
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="mb-2">
                                <label className="block text-lg text-gray-600 font-medium mb-2">Vendor Id <span className="text-red-500 ">*</span></label>
                                <Controller
                                    name="vendor_id"
                                    control={control}
                                    render={({ field }) => (
                        ...
                        <div className="bg-white rounded-lg shadow-2xl p-8" onClick={(e) => e.stopPropagation()}>
                            ...
                            <div className="mb-2">
                                ...
                                <Controller
                                    name="category_name"
                                    control={control}
                                    defaultValue={categoryData?.category_name || ''}  // Autofill with the selected category's name
                                    render={({ field }) => (
                                        <input
                                            type="text"
                                            placeholder="Enter Category Name"
                                ...
                                />
                                {errors.category_name && !categoryData?.name && <p className="text-red-500 text-sm">{errors.category_name.message}</p>}
                            </div>
                            ...
                            <div className="mb-2">
                                ...
                                <Controller
                                    name="vendor_id"
                                    control={control}
                                    defaultValue={categoryData?.vendor_id || ''}  // Autofill with the selected category's name
                                    render={({ field }) => (
                                        <input
                                            type="text"
                                            placeholder="Enter Category Name"
                                ...
                                />
                                {errors.vendor_id && !categoryData?.name && <p className="text-red-500 text-sm">{errors.vendor_id.message}</p>}
                            </div>
                            ...
                            <div className="mb-2">
                                ...
                                <input
                                    ...
                                    className="w-full border border-gray-300 rounded-md px-4 py-3 text-gray-800 focus:outline-none"
                                />
                                {errors.category_image && <p className="text-red-500 text-sm">{errors.category_image.message}</p>}
                                {/* {categoryData?.photo && (
                                    <div className="mt-4">
                                        <img
                    ...
                    <table className="w-full table-auto">
                        <thead>
                            <tr className="text-gray-600">
                                ...
                                <th className="px-6 py-4 text-left">Category Image</th>
                                    ...
                                    <tr key={category.id} className="border-t group">
                                        ...
                                        <td className="text-right">
                                            <div className="relative inline-flex items-center space-x-2">
                                                ...
                                                <div className="absolute right-20 flex space-x-6 opacity-0 group-hover:opacity-100 group-hover:flex transition-all duration-200">
                                                    <button
                                                        onClick={() => handleEditRow(category)}
