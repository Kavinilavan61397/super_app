...
export const categoryService = {
  ...
  getCategoryById: async (id) => {
    try {
      const response = await api.get(API_CONFIG.CATEGORY.DETAIL(id));
  ...
  createCategory: async (formData) => {
    try {
      const response = await api.post(API_CONFIG.CATEGORY.CREATE, formData, {
        headers: {
  ...
  updateCategory: async (id, formData) => {
    try {
      const response = await api.put(API_CONFIG.CATEGORY.UPDATE(id), formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
      });
  ...
  toggleStatus: async (id) => {
    try {
      const response = await api.patch(API_CONFIG.CATEGORY.TOGGLE_STATUS(id));
      return response.data;
    } catch (error) {
