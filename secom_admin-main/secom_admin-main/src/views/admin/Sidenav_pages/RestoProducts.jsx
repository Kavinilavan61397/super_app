...
function RestoProducts() {
  ...
  const fetchCategoryData = async () => {
    try {
      const response = await axios.get('https://yrpitsolutions.com/ecom_backend/api/get_all_restaurant_categories');
      setCategories(response.data); // Store the categories

      if (response.data && response.data.length > 0) {
        const selectedVendorId = response.data[0].vendor_id; // Get the vendor_id of the first category
        setVendorId(selectedVendorId); // Save the vendor_id to state
  ...
  const fetchProducts = async () => {
    try {
      const response = await axios.get('https://yrpitsolutions.com/ecom_backend/api/get_all_restaurant_product');
  ...
  const handleSelectChange = (selectedTags) => {
    setSelectedTags((prevSelected) => {
      if (!prevSelected.some((tag) => tag.id === selectedTags.id)) {
        return [...prevSelected, selectedTags]; // Add policy if not already selected
      }
  ...
  const handleRemoveSelectedTag = (tagId) => {
    setSelectedTags((prevSelected) =>
      prevSelected.filter((tag) => tag.id !== tagId) // Filter out the removed policy
    );
  };
  ...
  const handleFormSubmit = async (data) => {
    ...
    const existingProduct = getPaginatedData().find(product => product.product_name.toLowerCase() === ProductName.toLowerCase());
    ...
    formData.append("restaurant_sub_category_id", data.restaurant_sub_category_id);
    ...
    formData.append('price_with', 1000);

    formData.append("status", 1);

    selectedTags.forEach((tag) => {
      formData.append('tag_id[]', tag.id);  // Directly append the tag ID
    });
    ...
    try {
      ...
      const response = await axios.post(
        "https://yrpitsolutions.com/ecom_backend/api/admin/save_restaurant_product",
        formData,
        {
          headers: {
  ...
  const handleAddCategory = () => {
    ...
    setValue('restaurant_sub_category_id', '');
  ...
  const handleFormUpdate = async (data) => {
    ...
    try {
      ...
      const url = `https://yrpitsolutions.com/ecom_backend/api/admin/update_restaurant_product_by_id/${selectedProduct.id}`;
  ...
  const handleEditRow = (product) => {
    ...
    reset({
      restaurant_category_id: product.restaurant_category_id,
      restaurant_sub_category_id: product.restaurant_sub_category_id,
  ...
  useEffect(() => {
    if (selectedProduct) {
      reset({
        restaurant_category_id: selectedProduct.restaurant_category_id || '',
        restaurant_sub_category_id: selectedProduct.restaurant_sub_category_id || '',
  ...
  const handleDeleteConfirmation = async () => {
    ...
    try {
      ...
      await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_product_by_id/${rowIdToDelete}`, {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      for (let id of selectedRows) {
        await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_product_by_id/${id}`, {
          headers: { Authorization: `Bearer ${accessToken}` },
        });
      }
              ...
              <form onSubmit={handleSubmit(handleFormSubmit)}>
                ...
                <div className="mb-4">
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        render={({ field }) => (
                          <select
                            ...
                            onChange={(e) => {
                              ...
                              if (selectedCategory) {
                                setVendorId(selectedCategory.vendor_id); // Ensure vendor_id is set when category changes
                                field.onChange(e); // Ensure the value is updated in form state
                              }
                            }}
                          >
                    ...
                    <Controller
                      name="restaurant_sub_category_id"
                      control={control}
                    ...
                    <div className='relative'>
                            ...
                            {tags.length > 0 ? (
                              tags.map((tag) => (
                                <li
                                  ...
                                  className={`px-4 py-2 cursor-pointer hover:bg-gray-200 ${selectedTags.some(
                              ...
                              <li className="px-4 py-2 text-gray-500">No Tags available</li>
                            )}
                          </ul>
                        </div>
                      )}
                      <div className="mt-2 flex flex-wrap gap-2">
                        {selectedTags.map((tag) => (
                          <span key={tag.id} className="flex items-center px-4 py-2 rounded-full border border-gray-600">
              ...
              {getPaginatedData().length === 0 ? (
                ...
                getPaginatedData().map((product) => (
                  <tr key={product.id} className="border-t group">
                    ...
                    <td className="px-6 py-4">{product.restaurant_category?.category_name || ''}</td>
                    <td className="px-6 py-4">{product.restaurant_sub_category?.sub_category_name || ''}</td>
                    <td className="px-6 py-4">
                      <button
                        onClick={() =>
                          navigate(`/admin/restoproductvariation?product_id=${product.id}&category_id=${product.restaurant_category_id}&sub_category_id=${product.restaurant_sub_category_id}&vendor_id=${product.vendor_id}`)
                        }
