...
// Women's Kurti product data
const womensKurtiItems = [
  {
    id: 1,
    name: 'A-Line Printed Kurti',
    originalPrice: 2550,
    discountedPrice: 2040,
    image: 'https://via.placeholder.com/400/FF5733/FFFFFF?text=A-Line+Kurti',
    description: 'Elegant A-line kurti with vibrant prints, perfect for festive occasions.',
    rating: 4.6,
    isBestSeller: true,
    quantity: 2,
    category: "Women's Wear - Kurtis",
  ...
  {
    id: 2,
    name: 'Straight Embroidered Kurti',
    originalPrice: 2900,
    discountedPrice: 2320,
    image: 'https://via.placeholder.com/400/33FF57/FFFFFF?text=Straight+Kurti',
    description: 'Classic straight cut kurti with delicate embroidery, suitable for daily wear.',
    rating: 4.5,
    isBestSeller: false,
    quantity: 1,
    category: "Women's Wear - Kurtis",
  ...
  },
  {
    ...
    image: 'https://via.placeholder.com/400/3357FF/FFFFFF?text=Anarkali+Kurti',
    description: 'Flared Anarkali kurti with intricate work, ideal for special events.',
    rating: 4.9,
    isBestSeller: true,
    quantity: 1,
    category: "Women's Wear - Kurtis",
  ...
  {
    id: 4,
    name: 'Asymmetric Modern Kurti',
    originalPrice: 2800,
    discountedPrice: 2240,
    image: 'https://via.placeholder.com/400/FF33DA/FFFFFF?text=Asymmetric+Kurti',
    description: 'Fashion-forward asymmetric kurti, great for a contemporary look.',
    rating: 4.6,
    isBestSeller: false,
    quantity: 1,
    category: "Women's Wear - Kurtis",
    subCategory: 'Modern Kurtis',
    sizes: ['XS', 'S', 'M', 'L', 'XL'],
    brand: 'Global Desi',
    fabrics: ['Georgette'],
    design: 'Asymmetric',
  ...
  },
  {
    ...
    image: 'https://via.placeholder.com/400/33FFF3/FFFFFF?text=Pathani+Kurti',
    description: 'Comfortable Pathani style kurti with side slits, perfect for casual outings.',
    rating: 4.4,
    isBestSeller: false,
    quantity: 1,
    category: "Women's Wear - Kurtis",
    ...
    design: 'Pathani',
    length: 'Knee Length',
    neckType: 'Shirt Collar',
    sleeveType: 'Roll-up Sleeve',
  },
  {
    id: 6,
    ...
    image: 'https://via.placeholder.com/400/8D33FF/FFFFFF?text=Jacket+Kurti',
    description: 'Trendy jacket style kurti, suitable for a fusion look.',
    rating: 4.7,
    isBestSeller: true,
    quantity: 1,
    category: "Women's Wear - Kurtis",
  ...
  },
  {
    ...
    image: 'https://via.placeholder.com/400/FF8833/FFFFFF?text=Embroidered+Cotton+Kurti',
    description: 'Comfortable cotton kurti with delicate embroidery on the yoke.',
    rating: 4.5,
    isBestSeller: false,
    quantity: 1,
    category: "Women's Wear - Kurtis",
  ...
  {
    id: 8,
    name: 'Printed Rayon Kurti with Palazzo',
    originalPrice: 2700,
    discountedPrice: 2160,
    image: 'https://via.placeholder.com/400/33FF88/FFFFFF?text=Printed+Kurti+Palazzo',
    description: 'Stylish printed rayon kurti paired with comfortable palazzo pants.',
    rating: 4.6,
    isBestSeller: false,
    quantity: 1,
    category: "Women's Wear - Kurtis",
...
const ProductCard = ({ item, onQuickView, addToCart, addToWishlist, cartItems, wishlistItems }) => {
  const { name, originalPrice, discountedPrice, image, description, brand, sizes, fabrics, design, length, neckType, sleeveType } = item;
  ...
  >
    <FaEye className="w-4 h-4" />
      ...
      <div className="p-3 flex flex-col flex-grow">
        ...
        <p className="text-xs text-gray-600 mb-1">Fabric: {fabrics.join(', ')}</p>
        <p className="text-xs text-gray-600 mb-1">Design: {design}</p>
        <p className="text-xs text-gray-600 mb-1">Length: {length}</p>
        <p className="text-xs text-gray-600 mb-1">Neck Type: {neckType}</p>
        <p className="text-xs text-gray-600 mb-2">Sleeve Type: {sleeveType}</p>
...
function WomensKurtis() {
  ...
  const brands = [...new Set(womensKurtiItems.map((item) => item.brand))];
  const allAvailableFabrics = [...new Set(womensKurtiItems.flatMap((item) => item.fabrics))];
  const allAvailableCategories = [...new Set(womensKurtiItems.map((item) => item.subCategory))];
  const allAvailableDesigns = [...new Set(womensKurtiItems.map((item) => item.design))];
  const allAvailableLengths = [...new Set(womensKurtiItems.map((item) => item.length))];
  const allAvailableNeckTypes = [...new Set(womensKurtiItems.map((item) => item.neckType))];
  const allAvailableSleeveTypes = [...new Set(womensKurtiItems.map((item) => item.sleeveType))];
  ...
  const filterCategories = [
    ...
    { id: 'lengths', label: 'Length' },
    { id: 'neckTypes', label: 'Neck Type' },
  ...
  const handleFilterChange = (key, value) => {
    setFilterState((prev) => {
      ...
      if (['discount', 'brands', 'sizes', 'fabrics', 'categories', 'designs', 'lengths', 'neckTypes', 'sleeveTypes'].includes(key)) {
    ...
    return womensKurtiItems
      .filter((item) => {
        ...
        const matchesFabric = filterState.fabrics.length
          ? filterState.fabrics.some((fabric) => item.fabrics.includes(fabric))
          : true;
        const matchesCategory = filterState.categories.length
          ? filterState.categories.includes(item.subCategory)
          : true;
        const matchesDesign = filterState.designs.length
          ? filterState.designs.includes(item.design)
          : true;
        const matchesLength = filterState.lengths.length
          ? filterState.lengths.includes(item.length)
          : true;
        const matchesNeckType = filterState.neckTypes.length
          ? filterState.neckTypes.includes(item.neckType)
          : true;
        const matchesSleeveType = filterState.sleeveTypes.length
          ? filterState.sleeveTypes.includes(item.sleeveType)
          : true;

        return matchesDiscount && matchesBrands && matchesPrice && matchesBestSeller && matchesSize && matchesFabric && matchesCategory && matchesDesign && matchesLength && matchesNeckType && matchesSleeveType;
      })
      .sort((a, b) => {
  ...
  const renderFilterContent = () => {
    switch (selectedFilterCategory) {
            ...
            {allAvailableDesigns.map((design) => (
              <div key={design} className="flex items-center">
                <input
                  type="checkbox"
                  id={`design-${design}`}
                  checked={filterState.designs.includes(design)}
                  onChange={() => handleFilterChange('designs', design)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
                  aria-label={`Filter by ${design}`}
                />
                <label htmlFor={`design-${design}`} className="ml-2 text-sm text-gray-600">
                  {design}
                </label>
              </div>
            ))}
          ...
          <div className="space-y-2">
            {allAvailableLengths.map((length) => (
              <div key={length} className="flex items-center">
                ...
                />
                <label htmlFor={`length-${length}`} className="ml-2 text-sm text-gray-600">
                  {length}
                </label>
              </div>
            ))}
      ...
      case 'neckTypes':
        return (
          <div className="space-y-2">
            {allAvailableNeckTypes.map((neckType) => (
              <div key={neckType} className="flex items-center">
                <input
                  type="checkbox"
                  id={`neckType-${neckType}`}
                  checked={filterState.neckTypes.includes(neckType)}
                  onChange={() => handleFilterChange('neckTypes', neckType)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
                  aria-label={`Filter by ${neckType}`}
                />
                <label htmlFor={`neckType-${neckType}`} className="ml-2 text-sm text-gray-600">
                  {neckType}
                </label>
              </div>
            ))}
      ...
      case 'sleeveTypes':
        return (
          <div className="space-y-2">
            {allAvailableSleeveTypes.map((sleeveType) => (
              <div key={sleeveType} className="flex items-center">
                <input
                  type="checkbox"
                  id={`sleeveType-${sleeveType}`}
                  checked={filterState.sleeveTypes.includes(sleeveType)}
                  onChange={() => handleFilterChange('sleeveTypes', sleeveType)}
                  className="h-4 w-4 text-blue-600 rounded border-gray-300 focus:ring-blue-500"
                  aria-label={`Filter by ${sleeveType}`}
                />
                <label htmlFor={`sleeveType-${sleeveType}`} className="ml-2 text-sm text-gray-600">
                  {sleeveType}
                </label>
              </div>
            ))}
      ...
      <div className="flex-grow pt-16">
        <div className="max-w-screen-xl mx-auto px-4 py-4 pb-12">
          ...
          <div className="text-xs text-gray-600 mb-4">
            ...
            / <span className="font-semibold">Women's Kurtis</span> - {filteredAndSortedItems.length} Items
          </div>

          {/* Filter and Sort Controls */}
          <div className="flex flex-col sm:flex-row sm:flex-nowrap sm:justify-between sm:items-center gap-2 mb-4">
            <h2 className="text-lg font-bold text-gray-800 sm:text-sm">Women's Kurtis ({filteredAndSortedItems.length})</h2>
      ...
      {quickView && (
        ...
        >
          ...
          >
            ...
            <p className="text-xs text-gray-500 mb-1">Fabric: {quickView.fabrics.join(', ')}</p>
            <p className="text-xs text-gray-500 mb-1">Design: {quickView.design}</p>
            <p className="text-xs text-gray-500 mb-1">Length: {quickView.length}</p>
            <p className="text-xs text-gray-500 mb-1">Neck Type: {quickView.neckType}</p>
            <p className="text-xs text-gray-500 mb-2">Sleeve Type: {quickView.sleeveType}</p>
