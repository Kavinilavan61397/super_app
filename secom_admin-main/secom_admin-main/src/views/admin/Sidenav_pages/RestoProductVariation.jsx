...
function RestoProductVariation() {
    ...
    const fetchVariationData = async () => {
        ...
        try {
            const response = await axios.get(
                `https://yrpitsolutions.com/ecom_backend/api/restaurant-product-variation/by-product-id/${productId}`
            );
            setTableData(response.data.data.product_variations);
            setTotalItems(response.data.data.product_variations.length);
            console.log(response.data.data.product_variations)
        } catch (error) {
    ...
    useEffect(() => {
        if (searchQuery) {
            const filtered = tableData.filter((variation) =>
                variation.product_variation_name?.toLowerCase().includes(searchQuery.toLowerCase()) // Assuming variation_name is the field
            );
    ...
    const handleFormSubmit = async (data) => {
        ...
        const VariationName = data.product_variation_name;

        setLoading(true);

        const formData = new FormData();
        formData.append('vendor_id', vendorId);
        formData.append('restaurant_product_id', productId);
        formData.append('product_variation_name', data.product_variation_name);
        formData.append('product_variation_price', data.product_variation_price);
        formData.append('product_variation_quantity', data.product_variation_quantity);
        ...
        try {
            ...
            const url = 'https://yrpitsolutions.com/ecom_backend/api/admin/save_restaurant_product_variation';
    ...
    const handleFormUpdate = async (data) => {
        ...
        formData.append('restaurant_product_id', data.restaurant_product_id || selectedBrand.restaurant_product_id);
        formData.append('product_variation_name', data.product_variation_name || selectedBrand.product_variation_name);
        formData.append('product_variation_price', data.product_variation_price || selectedBrand.product_variation_price);
        formData.append('product_variation_quantity', data.product_variation_quantity || selectedBrand.product_variation_quantity);
        ...
        try {
            const accessToken = localStorage.getItem('OnlineShop-accessToken');
            const url = `https://yrpitsolutions.com/ecom_backend/api/admin/update_restaurant_product_variation_by_id/${selectedBrand.id}`;
    ...
    const handleAddBrand = () => {
        ...
        setValue('vendor_id', '');
        setValue('restaurant_product_id', '');
        setValue('product_variation_name', '');
        setValue('product_variation_price', '');
        setValue('product_variation_quantity', '');
        setValue('unit_name', '');
        setValue('weight', '');
        setValue('status', '');
        setValue('images[]', '');
    ...
    const handleEditRow = (variation) => {
        ...
        setValue('vendor_id', variation.vendor_id);
        setValue('restaurant_product_id', variation.restaurant_product_id);
        setValue('product_variation_name', variation.product_variation_name);
        setValue('product_variation_price', variation.product_variation_price);
        setValue('product_variation_quantity', variation.product_variation_quantity);
        setValue('unit_name', variation.unit_name);
        setValue('weight', variation.weight);
        setValue('status', variation.status);
    ...
    const handleDeleteConfirmation = async () => {
        ...
        try {
            ...
            await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_product_variation_by_id/${rowIdToDelete}`, {
                headers: { Authorization: `Bearer ${accessToken}` },
            });
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            for (let id of selectedRows) {
                await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_product_variation_by_id/${id}`, {
                    headers: { Authorization: `Bearer ${accessToken}` },
                });
            }
                        ...
                        >
                            ...
                            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6 mt-5">
                                ...
                                <div>
                                    ...
                                    />
                                    {errors.product_variation_price && <p className="text-red-500 text-sm">{errors.product_variation_price.message}</p>}
                                </div>
                            ...
                            <div className="mb-6 grid grid-cols-1 md:grid-cols-2 gap-6 mt-5">
                                ...
                                <div>
                                    ...
                                    />
                                    {errors.product_variation_quantity && <p className="text-red-500 text-sm">{errors.product_variation_quantity.message}</p>}
                                </div>
                            ...
                            {getPaginatedData().length === 0 ? (
                                ...
                                getPaginatedData().map((variation) => (
                                    <tr key={variation.id} className="border-t group relative">
                                        ...
                                        <td className="px-6 py-4">{variation.product_variation_name}</td>
                                        <td className="px-6 py-4">{variation.product_variation_price}</td>
                                        <td className="px-6 py-4">{variation.product_variation_quantity}</td>
                                        <td className="px-6 py-4">{variation.weight}</td>
...
export default RestoProductVariation;
