...
const primerProducts = [
  {
    id: 1,
    name: 'Absolute Mattifying Primer',
    originalPrice: 750,
    discountedPrice: 600,
    description: 'Matte primer for oil control and long-lasting makeup.',
    rating: 4.5,
    isBestSeller: true,
    brand: 'Lakmé',
    ...
    offers: [
      { code: 'PRIMER5', description: '₹5 off on first order', discount: 5 },
  ...
  {
    id: 2,
    name: 'Herbal Glow Primer',
    originalPrice: 650,
    discountedPrice: 520,
    description: 'Hydrating primer with herbal extracts for radiant skin.',
    rating: 4.6,
    isBestSeller: true,
    brand: 'Lotus Herbals',
  ...
  {
    id: 3,
    name: 'Velvet Touch Primer',
    originalPrice: 900,
    discountedPrice: 720,
    description: 'Smoothening primer for a flawless makeup base.',
    rating: 4.4,
    isBestSeller: false,
    brand: 'Colorbar',
  ...
  },
  {
    ...
    originalPrice: 800,
    discountedPrice: 640,
    description: 'Long-wear primer for all-day makeup hold.',
    rating: 4.7,
    isBestSeller: true,
    brand: 'Lakmé',
  ...
  {
    id: 5,
    name: 'White Lily Primer',
    originalPrice: 600,
    discountedPrice: 480,
    description: 'Hydrating primer for dry and sensitive skin.',
    rating: 4.3,
    isBestSeller: false,
    brand: 'Lotus Herbals',
    category: 'Hydrating Primer',
  ...
  {
    id: 6,
    name: 'Flawless Base Primer',
    originalPrice: 950,
    discountedPrice: 760,
    description: 'Pore-minimizing primer for smooth application.',
    rating: 4.5,
    isBestSeller: true,
    brand: 'Colorbar',
    category: 'Smoothing Primer',
  ...
  {
    id: 7,
    name: 'Absolute Illuminating Primer',
    originalPrice: 850,
    discountedPrice: 680,
    description: 'Radiance-boosting primer for a dewy finish.',
    rating: 4.6,
    isBestSeller: false,
    brand: 'Lakmé',
  ...
  {
    id: 8,
    name: 'Pure Defense Primer',
    originalPrice: 700,
    discountedPrice: 560,
    description: 'Protective primer with SPF for daily use.',
    rating: 4.4,
    isBestSeller: false,
    brand: 'Lotus Herbals',
  ...
  {
    id: 9,
    name: 'Perfect Radiance Primer',
    originalPrice: 880,
    discountedPrice: 704,
    description: 'Glow-enhancing primer for radiant makeup.',
    rating: 4.5,
    isBestSeller: true,
    brand: 'Colorbar',
    category: 'Illuminating Primer',
  ...
  },
  {
    ...
    originalPrice: 820,
    discountedPrice: 656,
    description: 'Matte primer for oil-free makeup look.',
    rating: 4.7,
    isBestSeller: true,
    brand: 'Lakmé',
    category: 'Matte Primer',
...
const PrimerHeader = ({ setSelectedCategory, selectedCategory }) => {
  const [isPrimerSubmenuOpen, setIsPrimerSubmenuOpen] = useState(false);
  const [isMobileMenuOpen, setIsMobileMenuOpen] = useState(false);

  const primerCategories = [
    { name: 'All', value: 'all' },
    { name: 'Matte Primer', value: 'Matte Primer' },
    { name: 'Hydrating Primer', value: 'Hydrating Primer' },
    { name: 'Smoothing Primer', value: 'Smoothing Primer' },
    { name: 'Illuminating Primer', value: 'Illuminating Primer' },
    { name: 'Long-Wear Primer', value: 'Long-Wear Primer' },
    { name: 'Protective Primer', value: 'Protective Primer' },
  ];
  ...
  const togglePrimerSubmenu = () => {
    setIsPrimerSubmenuOpen((prev) => !prev);
  };
      ...
      <div className="container mx-auto flex flex-col sm:flex-row justify-between items-center gap-2">
        ...
        <nav className={`flex flex-col sm:flex-row items-center gap-2 sm:gap-4 ${isMobileMenuOpen ? 'block' : 'hidden sm:flex'}`}>
          <ul className="flex flex-col sm:flex-row flex-wrap justify-center gap-2 sm:gap-4 w-full">
            {primerCategories.map((category) => (
              <li key={category.value} className="relative">
                {category.name === 'Matte Primer' ? (
                  <>
                    <button
                      className={`text-sm px-3 py-1 rounded-md ${
                        selectedCategory === category.value || primerVariants.includes(selectedCategory)
                          ? 'bg-white text-black'
                          : 'bg-black text-white hover:bg-gray-700'
                      }`}
                      onClick={togglePrimerSubmenu}
                      aria-label={`Toggle ${category.name} submenu`}
                    >
                      {category.name}
                      <svg
                        className={`inline-block ml-1 w-4 h-4 transform ${isPrimerSubmenuOpen ? 'rotate-180' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                      >
...
function Primers() {
  ...
  const brands = ['Lakmé', 'Lotus Herbals', 'Colorbar'];
  const priceRanges = [
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 font-sans">
      <PrimerHeader setSelectedCategory={setSelectedCategory} selectedCategory={selectedCategory} />
            ...
            <div className="flex-grow pt-20">
              <div className="max-w-screen-xl mx-auto px-4 py-4 pb-16 relative">
                <div className="text-sm text-gray-600 mb-4">
                  ...
                  <span className="font-semibold">Primers</span> - {filteredAndSortedItems.length} Items
                </div>

                <div className="flex flex-col sm:flex-row sm:flex-nowrap sm:justify-between sm:items-center gap-2 mb-4">
                  <h2 className="text-xl font-bold text-gray-800 sm:text-sm">Primers ({filteredAndSortedItems.length} Items)</h2>
                ...
                <main className="flex-grow">
                  ...
                  {filteredAndSortedItems.length === 0 && (
                    <p className="text-center text-gray-600 mt-8 text-sm">No primers found matching your criteria.</p>
                  )}
                </main>
              </div>
            </div>
