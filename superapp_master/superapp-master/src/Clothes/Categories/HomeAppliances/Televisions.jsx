...
function ProductCard({ product, onQuickView, addToCart, addToWishlist, cartItems, wishlistItems }) {
    ...
    return (
        <div className="border rounded-2xl p-3 bg-white shadow-sm flex flex-col items-center relative h-full">
            ...
            <button className={`bg-black hover:bg-grey:800 text-white px-4 py-2 rounded text-xs font-semibold w-full mt-auto mb-2 ${isInCart ? 'opacity-60 cursor-not-allowed' : ''}`} onClick={() => { if (!isInCart) { console.log('Add to Cart clicked', product, qty); addToCart(product, qty); }}} disabled={isInCart}>{isInCart ? 'Added to Cart' : 'Add to Cart'}</button>
        </div>
    );
}
    ...
    const fetchProducts = async () => {
        ...
        try {
            ...
            let filtered = (data.data || data).filter(
              p => (p.category === 'Televisions' || p.category.name === 'Televisions')
            );
            ...
            <div className='mt-24 mb-28 pb-8 px-4'>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 gap-4 mt-4">
                    {products.map(product => (
                        <ProductCard key={product.id} product={product} onQuickView={setQuickViewProduct} addToCart={handleAddToCart} addToWishlist={handleAddToWishlist} cartItems={cart?.items?.map(i => ({ ...i.product, id: i.product_id })) || []} wishlistItems={wishlist?.map(i => ({ ...i.product, id: i.product_id })) || []} />
                    ))}
                </div>
            </div>
            <Footer />
