...
function RestoStocks() {
    ...
    const fetchProductById = async () => {
        ...
        try {
            const response = await fetch(`https://yrpitsolutions.com/ecom_backend/api/getRestaurantProductVariationById/${productId}`);
            const data = await response.json();
            console.log(data);

            if (response.ok) {
                setProductName(data.data.product_variation_name);
                setProductDataId(data.data.restaurant_product_id)
                setQuantity(); // Default quantity (you can adjust this value)
                setVendorId(data.data.restaurant_product?.vendor_id);
                console.log(data.data.restaurant_product?.vendor_id)
            } else {
    ...
    const fetchStocks = async () => {
        console.log(productId);
        try {
            const response = await axios.get(`https://yrpitsolutions.com/ecom_backend/api/admin/getStockByRestaurantProductVariation/${productId}`);
    ...
    useEffect(() => {
        if (searchQuery.trim() === '') {
            ...
            const filtered = tableData.filter(brand =>
                brand.product_variation_name.toLowerCase().includes(searchQuery.toLowerCase())
            );
    ...
    const handleSubmit = async (e) => {
        ...
        formData.append('restaurant_product_id', ProductDataId);
        formData.append('product_variation_name', productName);
        formData.append('product_variation_quantity', quantity);

        try {
            const response = await axios.post(
                `https://yrpitsolutions.com/ecom_backend/api/admin/update_restaurant_product_variation_stock_by_id/${productId}`,
                formData,
                {
                    headers: {
    ...
    const handleDeleteConfirmation = async () => {
        ...
        try {
            const token = localStorage.getItem('OnlineShop-accessToken');
            await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_stock_management/${rowIdToDelete}`, {
    ...
    const handleBulkDelete = async () => {
        ...
        try {
            ...
            for (let id of selectedRows) {
                await axios.delete(`https://yrpitsolutions.com/ecom_backend/api/admin/delete_restaurant_stock_management/${id}`, {
        ...
        <div className="min-h-screen pt-6">
            ...
            <div className="bg-white p-8 rounded-lg shadow-lg w-full mt-6">
                ...
                <form onSubmit={handleSubmit}>
                    <div className="mb-4">
                        <label htmlFor="product_variation_name" className="block text-gray-700 font-medium mb-2">Product Variation Name:</label>
                        <input
                            type="text"
                            id="product_variation_name"
                            value={productName}
                    ...
                    <div className="mb-6">
                        <label htmlFor="product_variation_quantity" className="block text-gray-700 font-medium mb-2">Quantity:</label>
                        <input
                            type="number"
                            id="product_variation_quantity"
                            value={quantity}
